...
export const StaffInProgressTasksPage: React.FC = () => {
  ...
  const inProgressListAll = useMemo(() => {
    return tasks.filter((t) => {
      const st = t.status as string;
      return st === 'Accepted' || st === 'On the Way' || st === 'In Progress';
    });
  }, [tasks]);

  const metrics = useMemo(() => {
    const totalActive = inProgressListAll.length;
    const highCount = inProgressListAll.filter((t) => t.priority === 'High').length;
    const dueSoon = inProgressListAll.filter((t) => {
    ...
    return { totalActive, highCount, dueSoon, slaCompliancePercent };
  }, [inProgressListAll, tasks, now]);

  const uniqueLocations = useMemo(() => {
    const locSet = new Set<string>();
    inProgressListAll.forEach((t) => {
  ...
  const filteredTasks = useMemo(() => {
    return inProgressListAll.filter((t) => {
  ...
  }, [inProgressListAll, priorityFilter, categoryFilter, slaFilter, dateFilter, locationFilter, searchQuery, now]);
  ...
  return (
    <DashboardLayout title="In Progress — Field Staff">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-200 pb-5">
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-600 font-medium">
              Tasks currently being executed in the field.
            </p>
          </div>
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-gray-200 space-y-3">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(searchQuery || priorityFilter !== 'All' || categoryFilter !== 'All' || slaFilter !== 'All' || dateFilter !== 'All' || locationFilter !== 'All') && (
                <button
                  type="button"
              ...
              <form onSubmit={handleAddProgressNote} className="p-4 bg-slate-50 rounded-xl border border-gray-200 space-y-3 text-xs">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Add field update note (e.g. Arrived on site, clearing debris)..."
                      ...
                      <div className="border-2 border-dashed border-gray-300 rounded-xl p-3 text-center space-y-2 bg-gray-50/50 h-44 flex flex-col items-center justify-center">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 w-full max-w-xs">
                          <input
                            ...
                            accept="image/*"
                            capture="environment"
                            id="staff-inprogress-camera-input"
                            className="hidden"
                            onChange={(e) => {
                          ...
                          />
                          <label
                            htmlFor="staff-inprogress-camera-input"
                          ...
                          <input
                            type="file"
                            accept="image/*"
                            id="staff-inprogress-gallery-input"
                            className="hidden"
                            onChange={(e) => {
                              if (e.target.files && e.target.files[0]) {
                                setPhotoAfterFile(e.target.files[0]);
                          ...
                          />
                          <label
                            htmlFor="staff-inprogress-gallery-input"
