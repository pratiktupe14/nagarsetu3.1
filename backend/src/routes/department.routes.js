...
router.post('/', authenticateToken, requireRole(['admin', 'city_admin']), async (req, res) => {
  try {
...
router.put('/:id', authenticateToken, requireRole(['admin', 'city_admin']), async (req, res) => {
  try {
    const { id } = req.params;
    const { name, code, description } = req.body;
...
router.delete('/:id', authenticateToken, requireRole(['admin', 'city_admin']), async (req, res) => {
  try {
    const { id } = req.params;
...
router.post('/assign', authenticateToken, requireRole(['department_head', 'admin', 'city_admin', 'officer']), validateInput(assignStaffSchema), async (req, res) => {
  try {
    ...
    if (!isAdmin) {
      ...
      if (complaint.department_id && staff.department_id && !isStaffCompDeptMatch) {
