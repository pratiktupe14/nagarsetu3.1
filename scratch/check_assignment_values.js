...
async function check() {
  try {
    ...
    const actorRes = await query("SELECT id, name, role, department_id, email FROM users WHERE name ILIKE '%Rahul Kumar%'");
    ...
    const staffRes = await query("SELECT id, name, employee_id, department_id FROM field_staff WHERE employee_id = 'PWD-STF-001' OR name ILIKE '%Amit Patil%'");
